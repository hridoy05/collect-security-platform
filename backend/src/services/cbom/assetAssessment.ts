import { BROKEN_ALGORITHMS, QUANTUM_VULNERABLE } from '../crypto';

const QUANTUM_SAFE_ALGORITHMS = ['AES-256', 'SHA-256', 'SHA-3', 'CRYSTALS', 'ARGON2', 'BCRYPT'];
...
export function calculateDaysToExpiry(expiryDate?: string | Date | null): number | null {
  if (!expiryDate) {
    return null;
  }

  return Math.floor((new Date(expiryDate).getTime() - Date.now()) / MS_PER_DAY);
}

export function assessAsset(asset: AssetAssessmentInput) {
  const issues: string[] = [];
