import type { Prisma, PrismaClient, SecurityAlert } from '@prisma/client';
...
function toApiAlert(alert: SecurityAlert) {
  return {
    ...alert,
...
async function findMany(filters: AlertFilters = {}) {
...
async function create(data: Prisma.SecurityAlertCreateInput) {
...
async function findRecentBySourceIp(sourceIp: string, since: Date) {
...
async function findRecentForDashboard(since: Date) {
  return prisma.securityAlert.findMany({
