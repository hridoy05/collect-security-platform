...
async function findMany(filters: AssetFilters = {}) {
  ...
  return prisma.cryptoAsset.findMany({
    where,
    orderBy: [{ riskRating: 'asc' }, { daysToExpiry: 'asc' }]
  });
}

async function upsert(payload: Prisma.CryptoAssetUpsertArgs) {
...
async function getRiskSummary() {
  const [total, red, amber, green, quantumVulnerable, expiringSoon, expired, noRotation] =
    await Promise.all([
      ...
      prisma.cryptoAsset.count({
        where: { OR: [{ rotationPolicy: 'none' }, { rotationPolicy: null }] }
      })
    ]);
